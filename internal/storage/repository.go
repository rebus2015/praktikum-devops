...
type Repository interface {
	SetGauge(name string, val interface{}) (float64, error)
	IncCounter(name string, val interface{}) (int64, error)
	GetCounter(name string) (int64, error)
	GetGauge(name string) (float64, error)
	GetView() ([]memstorage.MetricStr, error)
}

func Create(cfg *config.Config) Repository {
	if cfg.StoreFile == "" {
